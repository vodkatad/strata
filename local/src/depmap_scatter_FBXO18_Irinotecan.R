...
data_f <- snakemake@input[["FBX_irino"]]
...
#data_f <- "/mnt/trcanmed/snaketree/prj/strata/local/share/data/FBH1_log2TPM_Expression_23Q2_IRINOTECAN_GDSC21088_IC50_91123.csv"
...
cor.test(data$`IC50_Irinotecan`, data$`FBXO18_log2(TPM+1)`)
sink()

y_breaks <- guess_ticks(data$`IC50_Irinotecan`, fixed_min=-3, fixed_max=3)
x_breaks <- guess_ticks(data$`FBXO18_log2(TPM+1)`, fixed_min=3, fixed_max=7)

p <- ggplot(data, aes(x=`FBXO18_log2(TPM+1)`, y=`IC50_Irinotecan`)) +  geom_point(size=1) + geom_smooth(method='lm', size=1)+ # ratio is ... 1 becomes 0.939
  unmute_theme+theme(legend.position="none") + xlab('FBXO18_log2(TPM+1)')+ylab('IC50_Irinotecan')+
